...
TEST_WITH_TEARDOWN(Convert_HotCold, TestConfig::ALL)
{
...

	{
		...
		CHECK(((unsigned int)&tmp.pCold - (unsigned int)&tmp) == 12);
	}
	...
	// class check
	TestHotCold *p = (TestHotCold *)pHotCold;
	CHECK(((unsigned int)&p->pHotHead - (unsigned int)pHotCold) == 0);
	CHECK(((unsigned int)&p->pColdHead - (unsigned int)pHotCold) == 4);
